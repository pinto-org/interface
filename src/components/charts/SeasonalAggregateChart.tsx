...
export interface SeasonalAggregateChartData {
    season: number;
    value: number;
    timestamp: Date;
}
...
const TOKEN_SUPPLY_TABS = ["Pinto", "PINTOWETH LP", "PINTOcbETH LP", "PINTOcbBTC LP", "PINTOUSDC LP LP"]
const TIME_TABS = ["1D", "1W", "1M", "3M", "YTD", "1Y", '2Y', "ALL"];


interface SeasonalAggregateChartProps {
    title: string;
    size: "small" | "large";
    activeTab: TimeTab;
    activeTokenSupplyTab: number;
    onChangeTab: (tab: TimeTab) => void;
    onChangeTokenSupplyTab: (tab: number) => void;
...
const SeasonalAggregateChart = ({
    title,
    size,
    activeTab,
    activeTokenSupplyTab,
    onChangeTab,
    onChangeTokenSupplyTab,
    useSeasonalResult,
    valueFormatter,
    tickValueFormatter,
    fillArea,
    ...
    className,
}: SeasonalAggregateChartProps) => {
    const [allData, setAllData] = useState<SeasonalAggregateChartData[] | null>(null);
    const [displayData, setDisplayData] = useState<SeasonalAggregateChartData | null>(null);
    ...
    const handleChangeTokenSupplyTabFactory = useCallback(
        (tab: number) => () => {
            onChangeTokenSupplyTab(tab);
            setAllData(null);
            setDisplayData(null);
        },
        [onChangeTokenSupplyTab],
        ...
        <div className={cn("rounded-[20px] bg-gray-1", className)}>
            <div className="flex px-6 justify-between">
                <div className="flex gap-4">
                    {TOKEN_SUPPLY_TABS.map((tabName: string, idx: number) => (
                        <div
                            key={tabName}
                            data-state={activeTokenSupplyTab === idx ? "active" : "inactive"}
                            onClick={handleChangeTokenSupplyTabFactory(idx)}
                            className={`rounded-full border border-pinto-gray-2 px-2 py-1 ${activeTokenSupplyTab === idx ? "text-pinto-green-4 sm:text-pinto-green-4 border-pinto-green-4 bg-pinto-green-1" : ""} pinto-sm sm:pinto-body-light cursor-pointer data-[state=inactive]:hover:text-pinto-green-4`}
                        >
                            {tabName}
                        </div>
                    ))}
...
export default SeasonalAggregateChart;
