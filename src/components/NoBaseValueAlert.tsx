import privyLogo from "@/assets/misc/Privy_Symbol_Black.png";
          ...
          <div className="bg-pinto-gray-1 border border-pinto-gray-2 rounded-[1.5rem] shadow-lg relative">
            ...
            <div className="flex flex-col gap-4 p-4 sm:gap-6 sm:p-6">
              <p className="text-sm sm:text-2xl pr-2">
                {connector?.type === "privy"
                  ? "Looks like you don't have any funds on Base. Fund your wallet via Privy, or send value to your wallet using an exchange."
                  : "Looks like you don't have any funds on Base. Bridge value from Ethereum, or send value to your wallet using an exchange."}
              </p>
              <div className="flex flex-col lg:flex-row items-center gap-4 w-full">
                {connector?.type === "privy" ? (
                  <Button
                    asChild
                    variant="outline"
                    className="bg-white text-black border-pinto-gray-2 hover:bg-gray-100 rounded-2xl h-12 sm:h-20 w-full sm:flex-1 px-4 sm:px-6 cursor-pointer"
                    onClick={() => {
                      if (!address) {
                        toast.error("Wallet address not found!");
                        return;
                      }
                      fundWallet({ address: address, options: { chain: base } });
                    }}
                  >
                    <div className="flex items-center gap-2 w-full">
                      <div className="flex-shrink-0 flex items-center gap-2">
                        <img src={privyLogo} alt="Base" className="h-6 sm:h-12" />
                      </div>
                      <span className="hidden sm:block text-sm sm:text-2xl text-left">Fund your wallet</span>
