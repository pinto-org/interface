...
import { RequisitionType } from "./core";
import { fetchTractorEvents } from "./events/tractor-events";
import { transformSowRequisitionEvent } from "./sowOrder";
import { SowBlueprintData } from "./sowOrder/tractor-sow-types";
import { Requisition, TractorRequisitionData, TractorRequisitionEvent } from "./types";
    ...
    if (calls.functionName === "advancedFarm" && calls.args[0]) {
      ...
      try {
        ...
        if (advancedPipeDecoded.functionName === "advancedPipe" && advancedPipeDecoded.args?.[0]?.length) {
          ...
          if (!entry) {
            console.debug("[Tractor/decodeSowTractorData] No entry found for function name:", decoded.functionName);
            return null;
          }

          const transformed = entry.transformer(decoded, chainId);

          if (!transformed) {
            console.debug(
              "[Tractor/decodeSowTractorData] Transformation failed for function name:",
