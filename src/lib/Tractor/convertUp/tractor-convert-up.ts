...
/**
 * Convert Up Grown Stalk Calculation
 *
 * When converting LP tokens to PINTO (convert up), users receive a grown stalk bonus.
 *
 * Formula:
 * - Bonus Grown Stalk = bonusStalkPerBdv × bdvConverted
 * - Actual Bonus = min(Bonus Grown Stalk, Existing Grown Stalk)
 * - Total Grown Stalk = Existing Grown Stalk + Actual Bonus
 *
 * Constraints:
 * - Bonus cannot exceed the existing grown stalk on the deposits being converted
 *
 * Example:
 * - Converting 1000 BDV with 0.5 bonus rate = 500 potential bonus stalk
 * - If deposits have 600 existing stalk → 500 bonus received (total: 1100)
 * - If deposits have 400 existing stalk → 400 bonus received (total: 800)
 *
 * @param bdvToConvert - Amount of BDV being converted
 * @param bonusGrownStalkPerBdv - Current gauge bonus rate
 * @param existingGrownStalk - Grown stalk on the deposits being converted
 * @returns Total grown stalk after conversion with bonus
 *
 * @note if no existing grown stalk is provided, the max bonus grown stalk is returned
 */
export function calculateConvertUpBonusGrownStalk(
  bdvToConvert: TV,
  bonusGrownStalkPerBdv: TV,
  existingGrownStalk?: TV,
) {
  // Calculate potential bonus
  const maxBonusGrownStalk = bdvToConvert.mul(bonusGrownStalkPerBdv).reDecimal(STALK.decimals);

  if (!existingGrownStalk) {
    return maxBonusGrownStalk;
  }

  // Bonus cannot exceed existing grown stalk
  const actualBonusGrownStalk = maxBonusGrownStalk.gt(existingGrownStalk) ? existingGrownStalk : maxBonusGrownStalk;

  return actualBonusGrownStalk;
}
