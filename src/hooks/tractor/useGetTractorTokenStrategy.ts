...
import { ExtendedTractorTokenStrategy, SowOrderTokenStrategy, getSowOrderTokenStrategy } from "@/lib/Tractor";
...
const useGetTractorTokenStrategyWithBlueprint = () => {
  ...
  const getTokenStrategy = useCallback((bp: BlueprintIsh): ExtendedTractorTokenStrategy | undefined => {
    ...
    if (strat === "SPECIFIC_TOKEN") {
      ...
      } satisfies ExtendedTractorTokenStrategy;
    }

    return {
      type: strat,
    } satisfies SowOrderTokenStrategy;
  }, []);
