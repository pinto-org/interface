...
export class Clipboard {
   ...
   */
  public static encode(pasteParams: PasteParamsByType[ClipboardType], etherValue: bigint = 0n) {
    let type: number;
    ...
    //@ts-ignore
    return encodeAbiParameters(types, encodeData);
  }
   ...
   * @returns
   */
  public static encodeSlot(returnDataIndex: number, copySlot: number, pasteSlot: number, etherValue: bigint = 0n) {
   ...
   */
  private static pack(params: PasteParams, preBytes: `0x${string}` = "0x0000") {
    return encodePacked(
      ["bytes2", "uint80", "uint80", "uint80"],
      [preBytes, BigInt(params[0]), BigInt(params[1]), BigInt(params[2])],
    );
  }
  ...
  private static prepare<T extends ClipboardType>(type: T, pasteParams: PasteParamsByType[T], etherValue: bigint) {
    ...
    switch (type) {
      case 0: {
        types.push({ name: "x", type: "bytes2" });
        encodeData.push(typeBytes);
        break;
      }
      case 1: {
        types.push({ name: "x", type: "bytes32" });
        encodeData.push(
    ...
    if (hasValue) {
      types.push({ name: "z", type: "uint256" });
